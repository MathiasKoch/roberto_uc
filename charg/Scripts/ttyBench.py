...
class TtyBench(object):
	...
	def reader(self):
		while self.reading:
			self.rx_data = self.rx_data + self.serial.read(1)
			n = self.serial.in_waiting
			if n:
				self.rx_data = self.rx_data + self.serial.read(n)
			#if data:
				#self.rx_count += len(self.rx_data)
				#self.rx_data = self.rx_data + data
				# TODO: verify data, find out how to split bytes into integers
				# is_coherent(data)
