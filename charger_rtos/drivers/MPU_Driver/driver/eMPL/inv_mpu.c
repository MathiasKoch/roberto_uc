 ...
 * mpu_write(unsigned char slave_addr, unsigned char reg_addr,
 *      unsigned char length, unsigned char const *data)
 * mpu_read(unsigned char slave_addr, unsigned char reg_addr,
 ...
 */
 ...
 #if defined EMPL_TARGET_STM32F3xx

 #include "hal.h"
 #include "mpu_functions.h"

 #if defined MPU_USE_SPI
 #define mpu_write   MPU_SPI_WRITE
 #define mpu_read    MPU_SPI_READ
 #elif defined MPU_USE_I2C
 #define mpu_write   MPU_I2C_WRITE
 #define mpu_read    MPU_I2C_READ
 #endif
...
int mpu_init(struct int_param_s *int_param)
{
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (mpu_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
...
int mpu_read_6500_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (mpu_read(st.hw->addr, 0x77, 2, &data[0]))
		return -1;
	if (mpu_read(st.hw->addr, 0x7A, 2, &data[2]))
		return -1;
	if (mpu_read(st.hw->addr, 0x7D, 2, &data[4]))
		return -1;
...
int mpu_read_6050_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (mpu_read(st.hw->addr, 0x06, 2, &data[0]))
		return -1;
	if (mpu_read(st.hw->addr, 0x08, 2, &data[2]))
		return -1;
	if (mpu_read(st.hw->addr, 0x0A, 2, &data[4]))
		return -1;
	accel_bias[0] = ((long)data[0]<<8) | data[1];
	accel_bias[1] = ((long)data[2]<<8) | data[3];
	accel_bias[2] = ((long)data[4]<<8) | data[5];
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
	if (mpu_read(st.hw->addr, 0x13, 2, &data[0]))
		return -1;
	if (mpu_read(st.hw->addr, 0x15, 2, &data[2]))
		return -1;
	if (mpu_read(st.hw->addr, 0x17, 2, &data[4]))
		return -1;
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (mpu_write(st.hw->addr, 0x13, 2, &data[0]))
        return -1;
    if (mpu_write(st.hw->addr, 0x15, 2, &data[2]))
        return -1;
    if (mpu_write(st.hw->addr, 0x17, 2, &data[4]))
        return -1;
    return 0;
}
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias) {
    ...
    if (mpu_write(st.hw->addr, 0x06, 2, &data[0]))
        return -1;
    if (mpu_write(st.hw->addr, 0x08, 2, &data[2]))
        return -1;
    if (mpu_write(st.hw->addr, 0x0A, 2, &data[4]))
        return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
    ...
    if (mpu_write(st.hw->addr, 0x77, 2, &data[0]))
        return -1;
    if (mpu_write(st.hw->addr, 0x7A, 2, &data[2]))
        return -1;
    if (mpu_write(st.hw->addr, 0x7D, 2, &data[4]))
        return -1;
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (mpu_read(st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (mpu_read(st.hw->addr, REG_6500_XG_ST_DATA, 3, regs)) {
    	if(debug)
    		log_i("Reading OTP Register Error.\n");
        return 0x07;
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    while (s < test.packet_thresh) {
		...
		if (mpu_read(st.hw->addr, st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
...
static int setup_compass(void)
{
    ...
    data[0] = BIT_mpu_read | st.chip_cfg.compass_addr;
